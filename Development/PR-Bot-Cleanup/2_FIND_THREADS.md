...
- [ ] Include unresolved threads from the bots configured in `[BOT_REVIEWERS]` (default `coderabbitai,greptile`) — match by login-name prefix to handle slight variations — but exclude human review threads entirely; if thread-level unresolved state is unavailable from Maestro tools, use `gh api graphql` or another minimal `gh` query and record that fallback in `{{AUTORUN_FOLDER}}/LOOP_{{LOOP_NUMBER}}_THREADS.md`.
...
- [ ] If zero unresolved bot threads are found, append an explicit zero-count summary for each bot in `[BOT_REVIEWERS]` to `{{AUTORUN_FOLDER}}/LOOP_{{LOOP_NUMBER}}_THREADS.md` so later documents can exit cleanly without guessing.
